...
- 1.3.4 (2026-09-10): hooks table rebuilt by analysis/hook_patterns.py with conversion per hook, campaign types, standouts, niches and title wording on the campaign analysis, which template to start from (source mix, top 20 Gleam templates, templates by industry).
...
- Every analysis script now reads the campaign analysis through one shared scope in `analysis/frame.py`, so all ten skills measure the same population: campaigns of 1,000 Entrants or more, with crypto, ambiguous and purchase-only campaigns removed. `the analysis notes` documents the tables, the known gaps and where each script stands.
