...
Reach beyond an existing audience is not one mechanic. The table in `references/hooks-and-themes.md` shows collaborations at 603 Entrants, 23% above typical, alongside referral actions, so never call either the only way to reach new people.
...
6. **Hand off.** The answer names the next piece of work in the reader's words, never by skill name: picking the Prize, choosing what Entrants do to enter, setting the dates. Write it as the next thing to do, not as an offer to do it. Add giveaway-winner-structure when the concept runs a series or several draws, and giveaway-promotion-plan when the concept is UGC or leans on reach the business does not yet have. This line is part of the output, not an optional extra: measured answers dropped it every time it was tested, and a concept with no named next step leaves the reader holding an idea and no way to start.
