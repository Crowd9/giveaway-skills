...
6. **Explain, with care.** Load `references/reading-results.md`. Impressions are unique per person per day, so daily actions and long runs push the Conversion Rate down without anything going wrong. Say which benchmark caveats apply before judging a number.
...
- A first campaign is ranked against the campaign-weighted figures like every other, so say plainly that the typical first campaign drew 382 Entrants where the all-campaign figure is 492, and read their result against 382 (`references/benchmarks.md`). Reporting a first campaign as below typical against a figure built from businesses on their eleventh is the most common way this report misleads someone.
