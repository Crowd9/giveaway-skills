...
1. **Read the export first.** When the user has a Gleam Actions export (one row per completed action, columns Email, Status, Action, Entries, Country, When), run `python3 scripts/gleam_export.py export.csv --actions-csv actions.csv`. It prints contestants, entries, actions completed, days, email signups, follows by network, referrals, the per-action counts, the country split and the referrer split, then the exact `review.py` command to run. Impressions are not in that file, so ask for the Reporting tab figure. Never print rows or names from the export.
...
- `scripts/gleam_export.py`: reads a Gleam Actions export into the numbers, the per-action CSV and an entrants CSV for the draw script. `--self-test` checks it.
