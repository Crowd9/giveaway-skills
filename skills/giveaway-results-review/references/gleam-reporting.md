...
The prompt can ask for the review as an interactive dashboard, and `scripts/dashboard.py` builds it: the written verdict, the numbers table and the three changes on the first screen, so a reader who never clicks still has the review, then one tab per Reporting tab (Overview, Traffic, Entry Methods, Viral, Audience, Outcomes) with every figure as the report script printed it, and a Levers tab. The reviewer writes the words to `words.json` and the script does the rest, so the page and the written review cannot disagree. Where the scripts cannot run, the written review is the answer, with the report's sections after it in the same order, and saying so is enough.
