...
def draft(a):
    L = [f"{a.name}: Terms and Conditions", "",
         f"1. Promoter. The promotion is run by {a.promoter}" + (f", {a.address}" if a.address else "") + " (the Promoter).",
         f"2. Entry period. Entries open at {a.open} and close at {a.close}. Entries received outside this period are invalid.",
         f"3. Eligibility. Entry is open to {a.eligible}. The following are not eligible: {a.exclude}.",
         "4. How to enter. Entrants complete the entry steps shown on the entry page. No purchase is necessary to enter. Where an optional step involves a purchase, a free entry route of equal weight is available. Entries that are incomplete, duplicated, automated, or made through multiple accounts are void.",
         f"5. Prize. {a.prize}. There are {a.winners} winner{'s' if a.winners != 1 else ''}. The prize is " + ("not transferable and no cash alternative is offered" if a.cash_alternative == "no" else "transferable and a cash alternative of equal value may be requested") + ". The Promoter may substitute a prize of equal or greater value if the stated prize becomes unavailable.",
         f"6. Winner selection. Winners are selected " + ("at random from all valid entries" if a.method == "random" else "by the Promoter's judges on the published criteria, and the judges' decision is final") + f" on {a.draw}." + (" The draw method is published in advance and the result can be verified from the published record." if a.method == "random" else ""),
         f"7. Notification. Winners are notified by {a.notify} within 3 days of selection and must respond within {a.reply_days} day{'s' if a.reply_days != 1 else ''} of notification. If a winner does not respond, cannot be verified as eligible, or declines the prize, the prize is forfeited and a replacement winner is selected the same way.",
         f"8. Verification. Winners may be asked to provide proof of identity, age and residence before the prize is released.",
         f"9. Delivery. Prizes are {a.delivery}. The Promoter is not responsible for prizes lost or damaged in transit once dispatched to the address the winner supplied." + (" Any tax, duty or charge arising from receipt of the prize is the winner's responsibility unless stated otherwise." if a.region != "none" else ""),
         f"10. Publicity. Winners consent to the Promoter publishing their {a.publish} for the purpose of announcing the result, and may withdraw that consent by contacting the Promoter.",
         "11. Personal information. Personal information collected is used to run the promotion, contact winners and deliver prizes, and is handled in accordance with the Promoter's privacy policy. Entrants may request access to or correction of their information by contacting the Promoter.",
         "12. Platforms. The promotion is in no way sponsored, endorsed, administered by, or associated with Instagram, Facebook, TikTok, X, YouTube, or any other platform on which it is promoted. Entrants release those platforms from any liability connected with the promotion.",
         "13. General. The Promoter may cancel, suspend or amend the promotion where required by law or where it cannot proceed as planned. The Promoter's decisions are final on matters the terms do not cover. Entry constitutes acceptance of these terms."]
